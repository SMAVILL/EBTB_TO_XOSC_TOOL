...
#     # Main GUI setup
#     root = tk.Tk()
#     root.title("EBTB_to_XOSC_Tool_ver1")
#
...
#     # log_file_path = 'generate_doc.log'
#     new_path_logs = os.path.join(selected_path,'report')
#     log_file_path = os.path.join(new_path_logs,'generate_doc.log')
#
...
#
#     # Initialize variables for collecting warnings
#     current_warnings = []
#
...
#     status = ["partially converted"] * len(Details)
#     # XOSC_path = ["C:/Your/Path/to/XOSC"]  # Update this path as needed
...
#         .data-section {{ margin-top: 20px; font-size: 18px; display: flex; align-items: center; }}
...
#             <span id="ebtb_folder">EBTB Folder: {EBTB_paths}</span>
#             <button class="copy-button" onclick="copyToClipboard('ebtb_folder')">Copy</button>
#         </div>
#         <div class="data-section">
#             <span id="xosc_path">XOSC Path: {XOSC_path+'/report'}</span>
#             <button class="copy-button" onclick="copyToClipboard('xosc_path')">Copy</button>
#         </div>
#
#         <table>
#             <thead>
...
#             <tbody>
#                 {''.join(
#         f"<tr><td>{info}</td><td>{xebtb}</td><td>{user}</td><td>{stat}</td><td>{warn}</td></tr>"
#         for info, xebtb, user, stat, warn in zip(XOSC_filename, EBTB_filename, Username, status, Details)
#     )}
#             </tbody>
#         </table>
#     </div>
...
#     html_path = os.path.join(new_path_logs, "webpage.html")
#     with open(html_path, "w") as file:
#         file.write(html_content)
#
#     # Open the HTML file in the web browser
#
#     #report_folder = selected_path
